...
Next, change the stack size. The stack size configured by the CubeMX will be used as for the
kernel and interrupt service routines. Stack as large as 4kB should be sufficient even for
...
At this stage you have integrated the CMRX kernel into your CubeMX project! Now
it is time to write the blinky application itself.
...
This can be fixed in CubeMX by opening `System Core` -> `NVIC` configuration page 
and selecting `Code Generation` tab. There, "Generate ISR handler" should be 
